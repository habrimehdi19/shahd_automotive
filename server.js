...
app.post("/api/invoices", (req, res) => {
  const { name, car, service, price, appointmentId } = req.body;
...
app.get("/api/appointments/pdf", (req, res) => {
  ...
  const cols = [
    ...
    { label: "Véhicule", width: 95 },
    { label: "Problème", width: 140 },
    { label: "Date/Heure", width: 90 },
  ...
  appointments.forEach((a, i) => {
    drawRow(
      [a.name, a.car, a.issue || "-", `${a.date} ${a.time}`, a.status],
      { stripe: i % 2 === 1 }
    );
  });
