...
// كتنستعمل من admin (dashboard) وتاني من العامل (/employee) باش الزبون يتوصل ف الحالتين.
function notifyCustomerStatusChange(appt, oldStatus) {
  ...
  // إلا الحالة ولات Terminé وكاينة فاتورة ديجا مرتبطة بهاد الموعد، صيفطها مباشرة
...
app.post("/api/invoices", (req, res) => {
  ...
  if (inv.appointmentId) {
    const linkedAppt = appointments.find((a) => Number(a.id) === Number(inv.appointmentId));
...
function buildInvoicePdfBuffer(inv) {
...
async function sendInvoiceToCustomer(inv, appt) {
  ...
  try {
    ...
    await bot.sendDocument(
      appt.chatId,
      buffer,
      { caption: `🧾 فاتورتك ديال ${inv.car}\nTotal : ${inv.price} MAD\n\nشكرا على الثقة فـ SHAHD AUTOMOTIVE! 🙏` },
