...
app.patch("/api/appointments/:id", (req, res) => {
  ...
  // ---- SMS/WhatsApp تلقائي ملي الحالة تبدل لـ "Terminé" ----
  if (appt.status === "Terminé" && oldStatus !== "Terminé" && appt.phone) {
    sendWhatsAppOrSMS(
      appt.phone,
      `✅ SHAHD AUTOMOTIVE\n\nمرحبا ${appt.name}، السيارة ديالك (${appt.car}) صلاحات وواجدة! تقدر تجي تاخدها.\n\n🔑 رقم التتبع: ${appt.code || "-"}`
    );
  }
