...
function setEmployeeCookie(res, token) {
  res.setHeader("Set-Cookie", `employee_session=${encodeURIComponent(token)}; HttpOnly; Path=/; SameSite=Lax; Max-Age=86400${process.env.NODE_ENV === "production" ? "; Secure" : ""}`);
}
function clearEmployeeCookie(res) {
  res.setHeader("Set-Cookie", "employee_session=; HttpOnly; Path=/; SameSite=Lax; Max-Age=0");
}
...
app.post("/employee/login", (req, res) => {
  ...
  const emp = employees.find(e => String(e.login || "").trim().toLowerCase() === login);
  ...
  if (!valid && emp.password && String(emp.password) === password) {
