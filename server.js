...
function verifyEmployeePassword(password, emp) {
  ...
  try { return crypto.timingSafeEqual(Buffer.from(hash, "hex"), Buffer.from(emp.passwordHash, "hex")); }
  catch { return false; }
}
...
function getEmployeeFromRequest(req) {
  ...
  const session = employeeSessions.get(cookies.employee_session || "");
  if (!session) return null;
  return employees.find(e => Number(e.id) === Number(session.employeeId)) || null;
}
function setEmployeeCookie(res, token) {
  res.setHeader("Set-Cookie", `employee_session=${encodeURIComponent(token)}; HttpOnly; Path=/employee; SameSite=Lax; Max-Age=86400`);
}
function clearEmployeeCookie(res) {
  res.setHeader("Set-Cookie", "employee_session=; HttpOnly; Path=/employee; SameSite=Lax; Max-Age=0");
}
...
app.post("/employee/login", (req, res) => {
  ...
  const emp = employees.find(e => String(e.login || "").toLowerCase() === String(login || "").trim().toLowerCase());
  if (!emp || !verifyEmployeePassword(password || "", emp)) {
    return res.status(401).json({ error: "Login ou password incorrect" });
  }
...
app.post("/employee/logout", (req, res) => {
  const cookies = parseCookies(req);
  employeeSessions.delete(cookies.employee_session || "");
...
app.patch("/api/employees/:id", (req, res) => {
  ...
  if (password) {
    const { salt, hash } = hashEmployeePassword(password);
    emp.passwordSalt = salt; emp.passwordHash = hash;
  }
  saveEmployees(employees);
...
app.delete("/api/employees/:id", (req, res) => {
  ...
  saveEmployees(employees);
  saveAppointments(appointments);
  for (const [token, session] of employeeSessions.entries()) if (Number(session.employeeId) === id) employeeSessions.delete(token);
  res.json({ ok: true });
});
